...
- **LaTeX science companion** — [docs/scaling_attention/](docs/scaling_attention/) — *Past the Quadratic Wall*, a ~85-page primer organised into four chapters (foundations through self-attention; the cost of attention and the scaling-laws era; engineering responses and subquadratic alternatives; SubQ, evaluation, and what comes next) plus three appendices. Tufte-book layout (same as `Algorithmic_Adventures`) set in **Alegreya Sans**; build with **XeLaTeX** (`./compile.sh`). See [docs/scaling_attention/README.md](docs/scaling_attention/README.md).
