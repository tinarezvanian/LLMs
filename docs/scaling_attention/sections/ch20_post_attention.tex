...
\paragraph{Diffusion language models (for transformer natives).}
Discrete diffusion treats generation as an iterative \emph{denoising} process: a
forward corruption (mask tokens, replace with uniform noise, or jump between
states) destroys the string, and a learned reverse kernel restores plausible text
\citep{hoogeboom2021multinomial,lou2024discrete}.
Unlike autoregressive left-to-right models, many diffusion steps can be
parallelised or scheduled adaptively, which is attractive for latency when the
number of steps is small.
The trade-off is training difficulty: the reverse process must approximate a
highly multi-modal denoising distribution, and likelihood evaluation is less
straightforward than next-token negative log-likelihood.
By 2025--2026 several ``Mercury / Inception-style'' demos showed that discrete
diffusion can reach deployable speeds on GPUs for chat-shaped workloads, but the
ecosystem (tool use, streaming UX, KV-style caching) still lags autoregressive
stacks---watch this space as objectives and schedulers mature.

\paragraph{What JEPA predicts.}
JEPA \citep{lecun2022jepa} trains an encoder \(f\) to map views of data into a
latent space where a small predictor \(g\) can forecast \emph{representations}
of masked regions from representations of visible context---without reconstructing
pixels or tokens directly.
Image-JEPA \citep{assran2023ijepa} and V-JEPA \citep{bardes2024vjepa} showed this
objective learns semantically meaningful video representations for downstream
tasks while ignoring high-frequency noise the way contrastive losses struggle to.
For text, the open research question is whether latent prediction can replace
next-token CE without losing the easy compositional semantics of autoregressive
Transformers; as of 2026 the commercial stack remains token-autoregressive, but
JEPA-style objectives are a credible research path beyond ``attention only.''
