...
\fillme{FILL-04-01}{Code listing: a single transformer block in PyTorch}{%
  ${\sim}50$-line listing implementing pre-LN block from
  Figure~\ref{fig:block}: \texttt{LayerNorm \texttt{->} MultiHeadAttention
  \texttt{->} residual \texttt{->} LayerNorm \texttt{->} MLP \texttt{->}
  residual}. Use only \texttt{torch.nn.Linear},
  \texttt{torch.nn.LayerNorm}, and \texttt{torch.nn.functional.softmax} —
  no \texttt{nn.MultiheadAttention} shortcut. Annotate tensor shapes on every
  line that reshapes. End with one paragraph mapping each line to the
  prose in this chapter.}{%
  This chapter; Vaswani et al.\ 2017 Algorithm~1; nanoGPT \texttt{model.py}
  for cross-checking.}{%
  ${\sim}50$ lines of code + one paragraph.}

\fillme{FILL-04-02}{Worked example: parameter accounting for Llama-3 8B}{%
  Tabulate, for the published Llama-3 8B configuration (\(L = 32\),
  \(\dmodel = 4096\), \(\heads = 32\), \(\dhead = 128\), GQA with 8 KV heads,
  \(d_{\text{ff}} = 14\,336\), SwiGLU, \(V = 128\,000\)): the parameter
  count of (i) embeddings, (ii) attention QKV+O per layer, (iii) FFN per
  layer, (iv) norms, (v) total. Confirm it sums to ${\sim}8.0\text{B}$.
  One footnote on what tied vs untied embeddings would change.}{%
  Llama-3 model card; configuration in HuggingFace
  \texttt{meta-llama/Meta-Llama-3-8B} repo.}{%
  Single 5-row \texttt{tabular} + 3 sentences of commentary.}

\fillme{FILL-04-03}{Sidebar: rotary position embeddings (RoPE) in 5 lines of math}{%
  Tufte sidenote-length sidebar that derives why
  \(q_i^{\text{rot}} \cdot k_j^{\text{rot}}\) depends only on \(j - i\) when
  query and key vectors are rotated in 2D subspaces by an angle proportional
  to position. End with one sentence on why this is the modern default
  (Llama, Mistral, Gemma) and one on what YaRN extends.}{%
  Su et al.\ 2021 (RoPE); Peng et al.\ 2023 (YaRN);
  \texttt{research/notes.md \S 4}.}{%
  ${\sim}150$ words; 1 small equation; render as a margin-width text block
  via \texttt{marginnote} or a regular paragraph.}
