\section{The transformer recipe}
\label{ch:transformer}

\newthought{This section} walks through a standard decoder-only transformer
block end to end, the way you would actually code it. The next section zooms
into the attention sub-layer; the chapter after that derives why that
sub-layer is the cost driver.

\subsection{The block, in one picture}
...
\begin{align}
  ...
  \label{eq:block}
\end{align}
That is a complete block.\sidenote{Two flavours: \emph{post-layer-norm (post-LN)} (the
original 2017 paper) puts layer normalisation (LN) \emph{after} the residual sum;
\emph{pre-layer-norm (pre-LN)} (now standard) puts LN \emph{before}, as written here.
...
families have dominated. (i) Learned absolute positions (BERT: Bidirectional Encoder Representations from Transformers; GPT-2: second-generation Generative Pre-trained Transformer):
...
rotation inside the attention's queries \(Q\) and keys \(K\), which is the de-facto modern
choice (Llama, Mistral, Gemma). RoPE composes cleanly with extrapolation
tricks like YaRN (Yet another RoPE extensioN) \citep{peng2023yarn}.}

\paragraph{Self-attention.} The hero of Section~\ref{ch:self-attention}.
...
7-billion-parameter model with \(d=4096\), \(L=32\) layers: attention \(\approx 32 \cdot 4 d^2
\approx 2.1\)B params, feed-forward network (FFN) \(\approx 32 \cdot 8 d^2 \approx 4.3\)B params,
...
\subsection{Why the transformer beat the LSTM}
...
defining problem of the field. Section~\ref{ch:why-quadratic} derives the
\(n^2\) cost carefully; Sections~\ref{ch:flashattention}--\ref{ch:kv-cache} document the engineering response;
Sections~\ref{ch:sparse}--\ref{ch:hybrids} cover the architectures that try to escape it.
