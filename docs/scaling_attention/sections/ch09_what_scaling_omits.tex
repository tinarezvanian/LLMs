...
(Chapter~\ref{ch:kv-cache}). A dense-attention deployment lands in the
\(10^{8}\)--\(10^{9}\) GPU-hour-per-year class at frontier per-token
economics; a \(\sim 10\times\) cheaper long-context architecture
(Chapters~\ref{ch:sparse}--\ref{ch:hybrids}: fixed-sparse, state-space, or routed attention) buys headroom by
...
The engineering chapters (Chapters~\ref{ch:flashattention}--\ref{ch:kv-cache}) are Camp~1.
The subquadratic-alternative chapters (Chapters~\ref{ch:sparse}--\ref{ch:hybrids}) are Camp~2.
Chapters~\ref{ch:quadrant}--\ref{ch:benchmarks} carry the 2$\times$2
