...
\subsection{Selection algorithms at frontier scale}

For content-dependent sparse attention to live in the bottom-right
quadrant of Section~\ref{ch:quadrant}, the selection function has to be
...
\subsection{Training infrastructure for the bottom-right quadrant}
...
\subsection{Evaluation that survives Goodhart}

Every benchmark in the long-context space has been gamed within months
of release.\sidenote{Original needle-in-a-haystack (NIAH) tests were trivial
once everyone trained on synthetic NIAH-style data; the move to RULER (multi-task synthetic long-context suite, \citealt{hsieh2024ruler}) and now
MRCR v2 (multi-document reasoning and retrieval benchmark, \citealt{chen2025mrcr}) is the response. Each new benchmark eventually stops
...
\subsection{Inference economics, not training economics}
