...
Chapters~\ref{ch:flashattention}--\ref{ch:kv-cache} (FlashAttention, MQA, GQA, paged caches) and the architectural
motivation for everything in Chapters~\ref{ch:sparse}--\ref{ch:hybrids} (state-space models, sparse attention,
hybrids).
...
is the topic of Chapters~\ref{ch:sparse}--\ref{ch:hybrids}.
...
turned LM training from research into engineering. Chapters~\ref{ch:kaplan}--\ref{ch:what-scaling-omits} are about that
...
of dollars \emph{per query class} at long context, which is why Chapters~\ref{ch:sparse}--\ref{ch:hybrids} treat
