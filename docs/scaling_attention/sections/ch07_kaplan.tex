...
\paragraph{Illustrative Kaplan extrapolation (with caveats).}
Kaplan-style scaling posits that cross-entropy improves predictably with model
size; one compact toy form is
\begin{equation}
...
datasets and recipes; treat \(L(N_{0})\) as an order-of-one anchor only.}
Plugging an illustrative ``GPT-4-scale'' dense budget \(N \approx 1.7\times
10^{12}\) (1.7T) gives
\((N_{0}/N)^{0.076} \approx 0.10^{0.076} \approx 0.84\), so
\(L(N) \approx 1.7 \times 0.84 \approx 1.4\)~nats---not a dramatic drop for a
\(10\times\) parameter increase, which is why the exponent matters more than
headline parameter counts.
This story is \emph{illustrative only}: \citet{hoffmann2022training} moved the
optimum toward many more tokens per parameter, real systems are often
mixture-of-experts, and public loss numbers for later models are not
comparable one-to-one to WebText nats. The point is structural: a smooth
power law can make extrapolation look like deterministic forecasting while hiding that
data and routing—not \(N\) alone—dominate production behaviour.

\paragraph{Emergent abilities: claims and corrections.}
\citet{wei2022emergent} catalogued sharp jumps on benchmarks once models crossed
scale thresholds—tasks look ``impossible'' until they suddenly aren't.
\citet{schaeffer2023emergentmirage} argued many jumps disappear under smoother
metrics or continuous skill measurements: perceived emergence tracks metric
discontinuity more than mysterious phase transitions.
The honest synthesis for scaling-law discourse is middle-ground: loss curves are
smooth while downstream passes/failures sit on thresholds; nonlinear capability
can surface without contradicting smooth optimisation dynamics---especially when
