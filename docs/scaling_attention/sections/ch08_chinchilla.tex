\section{Chinchilla: compute-optimal training}
\label{ch:chinchilla}
...
\subsection{The isoflops surface}
...
\(2 N D\) floating-point operations (FLOPs); backward pass is twice that; the constant absorbs ``per
...
= 2\times 10^{12}\) tokens, i.e.\ two trillion tokens seen during
...
roughly \(6.7\times 10^{5}\) GPU-hours, i.e.\ order \(10^{6}\) device-hours
...
Mistral, Gemma, Mixtral, Qwen, and Phi: every public model since reflects this
correction in some form.

\subsection{Bridge to long context}
...
document at inference time, the costs from Section~\ref{ch:why-quadratic}
...
attention costs (Sections~\ref{ch:flashattention}--\ref{ch:hybrids}) dominate serving bills.

That gap is what Section~\ref{ch:what-scaling-omits} is about, and what
