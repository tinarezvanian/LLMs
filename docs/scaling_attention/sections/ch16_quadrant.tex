...
\paragraph{Where DeepSeek Sparse Attention sits.}
DeepSeek's DSA \citep{deepseek2024dsa} routes tokens through learned sparse patterns
so each query scores a \emph{subset} of keys, restoring some content-dependence of
full attention while cutting \emph{average} FLOPs.
Because the router can still admit many pairs in adversarial patterns---and
because GPU kernels often densify irregular sparsity---worst-case behaviour can
remain much closer to quadratic than the clean \(\mathcal{O}(n)\) story of fixed
windows or convolutions.
On the Chapter~\ref{ch:quadrant} map, DSA sits in the upper half: data-driven
routing without the guaranteed-linear guarantee of a bottom-right SSA ideal.
