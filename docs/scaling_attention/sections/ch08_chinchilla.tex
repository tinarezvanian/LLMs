...
\paragraph{Worked example: Chinchilla tokens for a 100B model.}
The Chinchilla recipe recommends \(D \approx 20 N\) tokens for \(N\) non-embedding
parameters. Taking \(N = 10^{11}\) (100B) gives \(D \approx 2\times 10^{12}\)
tokens---two trillion tokens seen during training.
Equation~\ref{eq:flops} yields
\[
...
workloads (\(5\times 10^{14}\) FLOP/s), one device delivers
\(5\times 10^{14}\times 3600 \approx 1.8\times 10^{18}\) FLOPs per hour, so
\(C\) corresponds to roughly \(1.2\times 10^{24} / 1.8\times 10^{18} \approx
6.7\times 10^{5}\) GPU-hours---order \(10^{6}\) device-hours before counting
failed runs, checkpoint restarts, and evaluation.
At a stylised \(\$\,3\) per GPU-hour, that training phase alone is multiple
millions of dollars in \emph{rental} cost; frontier runs additionally amortise
clusters across thousands of GPUs and engineer-years. Published frontier budgets
(e.g.\ nine-figure estimates discussed alongside GPT-4-class systems) aggregate
hardware, power, networking, and iteration overhead beyond this toy multiply.
...
Stylised rental @ \(\$\,3/\mathrm{hr}\) & \(\mathcal{O}(\$\,\mathrm{millions})\) \\
\bottomrule
\end{tabular}
...
Chinchilla fixes \((N,D)\) given a \emph{training} compute envelope.
\citet{sardana2024beyond} observe that serving dominates lifetime spend for many
products: train once, answer billions of queries.
Imagine \(10^{10}\) requests each carrying \(10^{3}\) input tokens---\(10^{13}\)
decode tokens at inference versus \(2\times 10^{12}\) training tokens above.
Even tiny per-token savings from a slightly smaller, slightly longer-trained model
compound across those \(10^{10}\) calls; past a breakpoint, extra training data on
the smaller checkpoint buys cheaper inference forever.
The arithmetic is qualitative---exact crossover depends on quality constraints,
hardware margins, and routing---but it explains why deployment-first teams may
``over-train'' relative to textbook Chinchilla when \(N^{2}\) attention at long
context dominates serving bills (Part~IV--V).
