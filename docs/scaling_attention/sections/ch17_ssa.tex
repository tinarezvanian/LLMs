\section{Subquadratic Sparse Attention}
\label{ch:ssa}
...
bottom-right quadrant of Section~\ref{ch:quadrant}, and what is honestly
known versus what is still proprietary.\sidenote{Primary source for this
chapter: Subquadratic's (SubQ's) technical post \citep{subq2026ssa}, supplemented
by the company's blog and demos. Where claims are vendor-reported, that
is said out loud.}

\subsection{The mechanism, in one sentence}
...
\(\bigO(n^2)\) per layer, which is exactly what we are trying to avoid.
...
\subsection{What this buys you, on paper}
...
\subsection{What it costs in correctness}
...
\subsection{Where SSA sits relative to the alternatives}

In the 2$\times$2 frame from Section~\ref{ch:quadrant}: bottom-right.
Linear-cost (or near-linear-cost), content-dependent routing. Distinct
from:

\begin{itemize}
\item Fixed-pattern sparse (Section~\ref{ch:sparse}); same asymptote,
position-fixed routing.
\item State-space models (Section~\ref{ch:ssm}); same asymptote,
content-dependent dynamics but \emph{lossy compression} of the past into
a finite state.
\item Linear attention / Hyena (Section~\ref{ch:linattn}); better-than-
quadratic asymptote, but expressivity-limited routing kernels.
\item Hybrids (Section~\ref{ch:hybrids}); mostly position-fixed mixers
