...
transformer: FlashAttention, MQA, GQA, paged caches, RoPE/YaRN, and every
mitigation in Sections~\ref{ch:flashattention}--\ref{ch:kv-cache}, lives here. The asymptote is \(n^2\); the
...
of Sections~\ref{ch:sparse}--\ref{ch:hybrids} live here: fixed-pattern sparse (Section~\ref{ch:sparse}) and
state-space / convolutional models (Sections~\ref{ch:ssm}--\ref{ch:linattn}).
...
major architecture in Sections~\ref{ch:sparse}--\ref{ch:hybrids} occupies a different quadrant; SSA / SubQ
