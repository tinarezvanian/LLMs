\section{The KV cache and its band-aids}
\label{ch:kv-cache}
...
\subsection{Why caching is necessary}
...
cache drops from 0.5 MB to ${\sim}16$ KB.\sidenote{Quality on standard benchmarks typically
degrades a few percentage points; the interpretation
is that multi-head attention (MHA) gave each of the 32 heads its own keys and values, and collapsing
them to multi-query attention (MQA) throws some of that diversity away.}
...
Section~\ref{ch:hybrids} attack the same wall by mixing in linear-time
blocks.

\subsection{Paged KV (vLLM)}
...
\(L \cdot w\).\sidenote{For \(L = 32\) transformer layers, \(w = 4096\) sliding-window width, effective receptive
...
The SWA trade-off is precisely the one we will revisit in Sections~\ref{ch:sparse}--\ref{ch:hybrids}: cheap,
but the routing is fixed by position, so the model literally cannot see
information outside the window without help.

\subsection{RoPE, YaRN, and position extrapolation}
