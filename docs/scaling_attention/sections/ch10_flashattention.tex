\section{FlashAttention: same math, much better hardware story}
...
\subsection{The GPU memory hierarchy in one paragraph}
...
streaming multiprocessor (SM), but \emph{much} faster, with bandwidth on the
order of 19 TB/s.
...
\subsection{The standard implementation's mistake}
...
\subsection{What FlashAttention does}
...
\subsection{What it costs and what it doesn't}
...
\subsection{Why this matters for the rest of the book}
...
speedup at 1M tokens (Section~\ref{ch:benchmarks}), the baseline is
...
why the architectural alternatives in Sections~\ref{ch:sparse}--\ref{ch:hybrids} get serious attention now,
...
Sections~\ref{ch:sparse}--\ref{ch:hybrids} architectures.
