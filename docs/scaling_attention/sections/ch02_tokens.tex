\section{Tokens: the unit of work}
\label{ch:tokens}

\newthought{In Section~\ref{ch:what-lm}} we wrote ``each \(x_t\) is one
...
\subsection{Why characters and why not}
...
like ByT5 (``byte-level T5'', i.e.\ byte-level Text-to-Text Transfer Transformer) take this seriously.}
...
Section~\ref{ch:why-quadratic}), that 7$\times$ becomes a 50$\times$ compute
penalty per layer.

\subsection{Why words and why not}
...
\subsection{Subword tokenisation in one page}
...
LLMs.\sidenote{See \citep{sennrich2016bpe} for the original natural language processing (NLP) application
...
to the \texttt{bert-base-uncased} vocabulary (BERT: Bidirectional Encoder Representations from Transformers) and the famous ``\code{\#\#}''
...
SentencePiece (unigram subword tokeniser; often abbreviated SPM in tooling) is the reason Llama-style models can tokenise Japanese without
a custom segmenter.}

\subsection{Why this affects everything downstream}
...
\subsection{Embeddings: turning tokens into vectors}
...
\subsection{The same line, three tokenisers}
...
\subsection{Back-of-the-envelope token economics}
...
% Constrain width to the main text block so the margin caption is not painted over.
\begin{tabularx}{\linewidth}{@{} >{\raggedright\arraybackslash}X >{\raggedleft\arraybackslash}p{2.35cm} >{\centering\arraybackslash}p{2.45cm} >{\raggedleft\arraybackslash}p{2.45cm} @{}}
\toprule
...
users.\sidenote{See also fragmentation studies on multilingual OpenAI \texttt{cl100k\_base}
(as used in GPT-4--class models via \texttt{tiktoken}) and Llama tokenisers; the qualitative point is stable even when exact factors
shift.}

The next section is about how, before the transformer, people stitched these
